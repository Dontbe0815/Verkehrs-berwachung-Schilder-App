...
function ZoneRow(props: {
  ...
  const { z, role, canDelete, canCreate, openId, setOpenId, deleteZone, updateZone } = props;
    ...
    <div className="rounded-3xl border border-zinc-800 bg-zinc-950/40 p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" onClick={() => setOpenId(open ? null : z.id)} disabled={role === "spectator"}>
      ...
      {open ? (
        <div className="mt-4 space-y-3 rounded-3xl border border-zinc-800 bg-zinc-950/30 p-4">
          <div>
            <Label>Name</Label>
            <Input value={z.name} onChange={(e) => updateZone(z.id, { name: e.target.value })} disabled={!canCreate} />
          </div>
          <div>
            <Label>Beschreibung</Label>
            <Input value={z.description ?? ""} onChange={(e) => updateZone(z.id, { description: e.target.value.trim() ? e.target.value : null })} disabled={!canCreate} />
          </div>
          <div>
            <Label>Gültigkeit</Label>
            <Input value={z.validity ?? ""} onChange={(e) => updateZone(z.id, { validity: e.target.value.trim() ? e.target.value : null })} disabled={!canCreate} placeholder="Mo–Fr 7–18 Uhr" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="mt-1 flex items-center justify-between rounded-2xl border border-zinc-800 bg-zinc-950/40 px-3 py-2">
                ...
                <Switch
                  checked={z.isTemporary}
                  onChange={(v) => updateZone(z.id, { isTemporary: v, expiresAt: v ? (z.expiresAt ?? null) : null })}
                  disabled={!canCreate}
                />
              </div>
            </div>
            <div>
              ...
              <Input
                type="date"
                value={z.expiresAt ?? ""}
                onChange={(e) => updateZone(z.id, { expiresAt: e.target.value || null })}
                disabled={!canCreate || !z.isTemporary}
              />
          ...
          <div>
            <Label>Regeln / was gilt in der Zone?</Label>
            <Textarea rows={4} value={z.rules ?? ""} onChange={(e) => updateZone(z.id, { rules: e.target.value.trim() ? e.target.value : null })} disabled={!canCreate} />
          </div>
        </div>
      ) : null}
    </div>
